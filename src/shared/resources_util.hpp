#ifndef CUSTOM_CEF_APP_RESOURCES_UTIL_HPP
#define CUSTOM_CEF_APP_RESOURCES_UTIL_HPP
...
namespace shared
{
	...
	// 1. Add the ID value in */resources_files/win/resource.h
	// 2. Add the ID to file path mapping in */resources_files/win/resource.rc
	// 2. Add the |resource_path| to ID mapping in */resource_util_win_impl.cc.
	int GetResourceId(const std::string& resource_path);

	// Create a new provider for loading BINARY resources_files on Windows. Only URLs
